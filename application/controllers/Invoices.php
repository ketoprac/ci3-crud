<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Invoices extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model("invoice_model");
        $this->load->library('form_validation');
        $this->load->model("user_model");
        if($this->user_model->isNotLogin()) redirect(site_url('admin'));
    }

    public function index()
    {
        $data["invoices"] = $this->invoice_model->getAll();
        $this->load->view("invoice/list", $data);
    }

    public function add()
    {
        $invoice = $this->invoice_model;
        $validation = $this->form_validation;
        $validation->set_rules($invoice->rules());

        if ($validation->run()) {
            $invoice->save();
            $this->session->set_flashdata('success', 'Berhasil disimpan');
        }

        $this->load->view("invoice/new_form");
    }

    public function edit($id = null)
    {
        if (!isset($id)) redirect('invoices');
       
        $invoice = $this->invoice_model;
        $validation = $this->form_validation;
        $validation->set_rules($invoice->rules());

        if ($validation->run()) {
            $invoice->update();
            $this->session->set_flashdata('success', 'Berhasil disimpan');
        }

        $data["invoice"] = $invoice->getById($id);
        if (!$data["invoice"]) show_404();
        
        $this->load->view("invoice/edit_form", $data);
    }

    public function delete($id=null)
    {
        if (!isset($id)) show_404();
        
        if ($this->invoice_model->delete($id)) {
            redirect(site_url('invoices'));
        }
    }
}